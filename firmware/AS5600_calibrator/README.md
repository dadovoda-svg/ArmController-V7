...
* Applies a startup acceleration ramp when the motor begins moving
...
* `KEY_SCAN_MS = 20`
* `STEP_START_PERIOD_US = 400`
* `STEP_RUN_PERIOD_US = 50`
* `STEP_ACCEL_TIME_MS = 500`
...
* Releasing the button stops the motor immediately
...
Every motor start uses a short speed ramp:

* motion starts at `STEP_START_PERIOD_US`
* the timer period is reduced linearly until it reaches `STEP_RUN_PERIOD_US`
* the ramp duration is `STEP_ACCEL_TIME_MS`
* stopping is still immediate, with no deceleration ramp
...
Step pulses are generated by the ESP32 timer, and motor startup is ramped from a slower initial period to the final run period.

Current timing constants:

```cpp
constexpr uint32_t STEP_START_PERIOD_US = 400;
constexpr uint32_t STEP_RUN_PERIOD_US = 50;
constexpr uint32_t STEP_ACCEL_TIME_MS = 500;
```

The timer is initially started with the current step period and then updated during acceleration.

Final run speed is based on:
...
At the final run period, `T = 50 us`:

* STEP frequency is about **10 kHz**

At startup, `T = 400 us`:

* STEP frequency starts at about **1.25 kHz**

If you want to change the behavior:

* reduce `STEP_RUN_PERIOD_US` for a higher final speed
* increase `STEP_RUN_PERIOD_US` for a lower final speed
* increase `STEP_START_PERIOD_US` for a gentler startup
* increase `STEP_ACCEL_TIME_MS` for a longer acceleration ramp
